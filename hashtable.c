 ...
 */
void insertData(HashTable *table, void *key, void *data) { // FINISHED
  // -- TODO --
  // HINT:
  ...
  struct HashBucket *p = table->data[idx];
  struct HashBucket *t = p;
  p = malloc(sizeof(struct HashBucket));
  p->key = key;
  p->data = data;
  p->next = t;
  table->data[idx] = p;
 ...
 */
void *findData(HashTable *table, void *key) { // FINISHED
  // -- TODO --
  // HINT:
  // 1. Find the right hash bucket with table->hashFunction.
  // 2. Walk the linked list and check for equality with table->equalFunction.
  unsigned int idx = table->hashFunction(key);
  struct HashBucket *p = table->data[idx];
  while (p) {
    if (table->equalFunction(key, (char*)p->key)) return key;
    // if (table->equalFunction((char*)p->key, key)) return key;
    p = p->next;
  }
  return NULL;
}
